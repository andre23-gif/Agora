/* === AG_EDT_PAGE_REWRITE_V1 =====================================
   Page : Emploi du Temps (EDT) — SANS MODÈLE
   Métier :
     - 1 semaine = 1 document (edt_weeks + edt_cells)
     - semaine visible même vide
     - modification directe de la semaine active
     - "Valider" = enregistrer semaine active + appliquer aux semaines cochées (si cochées)
   Dépendances :
...
function cellText(jour, creneau) {
  ...
  const v = semaineActive.grid.get(key);
...
export async function renderEmploiDuTemps() {
  ...
  // Charger index semaines si pas encore fait
 ...
 document.querySelectorAll(".edt-weekcheck[data-iso]").forEach(cb => {
  ...
  if (valider) valider.onclick = async () => {
    try {
      ...
      // 2) appliquer si cibles cochées (hors source, ou incluant source, au choix : ici on exclut source)
...
async function ouvrirModal(jour, creneau) {
  ...
  const key = `${jour}|${creneau}`;
  const current = semaineActive.grid.get(key) || { classe_id: null, groupe: null };
  ...
  document.getElementById("edtOk").onclick = async () => {
    ...
    if (!classe_id) {
      semaineActive.grid.delete(key);
    } else {
      // récupérer nom depuis cache options (format label contient nom + groupe)
      ...
      semaineActive.grid.set(key, { classe_id, classe_nom: nom, groupe });
    }

    syncState = "dirty";
    semaineActive.status = "dirty";
    close();
    await refresh();
  };
}
